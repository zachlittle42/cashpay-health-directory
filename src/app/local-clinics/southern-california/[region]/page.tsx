...
import { SOCAL_REGIONS, getRegionBySlug, LocalClinic } from '@/lib/socal-clinics-data';

// Generate static params for all regions
export function generateStaticParams() {
  return SOCAL_REGIONS.map((region) => ({
    region: region.slug,
  }));
}
...
export function generateMetadata({ params }: { params: { region: string } }): Metadata {
  ...
  return {
    ...
    alternates: { canonical: `/local-clinics/southern-california/${params.region}` },
    keywords: [
...
export default function RegionPage({ params }: { params: { region: string } }) {
  ...
  const jsonLd = {
    ...
    url: `https://vitalityscout.com/local-clinics/southern-california/${region.slug}`,
        ...
        <div className="bg-gray-50 border-b border-gray-200">
          <div className="mx-auto max-w-6xl px-4 py-3">
            <nav className="text-sm text-gray-600">
              ...
              <span className="mx-2">→</span>
              <Link href="/local-clinics/southern-california" className="hover:text-blue-600">Southern California</Link>
              <span className="mx-2">→</span>
        ...
        <section className="bg-gray-50 px-4 py-12">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-5">
              {SOCAL_REGIONS.filter(r => r.slug !== region.slug).map((r) => (
                <Link
                  key={r.slug}
                  href={`/local-clinics/southern-california/${r.slug}`}
                  className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-lg hover:border-orange-300 transition"
                >
