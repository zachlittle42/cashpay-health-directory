...
// Shared renderer for a Longevity & Performance e-commerce category
// (e.g. /cgm, /longevity-rx). Data-driven from src/lib/performance-products.ts
// so adding a category is data + a thin route wrapper, no new layout.

function ProductCard({ product }: { product: PerfProduct }) {
  return (
...
export default function PerfCategoryPage({ slug }: { slug: PerfCategorySlug }) {
  const category = getPerfCategory(slug);
  const products = getPerfProductsByCategory(slug);
  ...
  const webPageSchema = {
    ...
    url: `https://vitalityscout.com/${slug}`,
    inLanguage: 'en-US',
      ...
      <div className="border-b border-gray-200 bg-gray-50">
        <div className="mx-auto max-w-5xl px-4 py-3">
          <nav className="text-sm text-gray-600">
            ...
            <Link href="/longevity-performance" className="hover:text-emerald-600">Longevity &amp; Performance</Link>
            <span className="mx-2">→</span>
      ...
      <section className="bg-emerald-50 px-4 py-12">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Link href="/longevity-performance" className="rounded-lg border border-emerald-200 bg-white p-6 hover:shadow-md transition-shadow">
              <div className="flex items-start gap-3">
                <span className="text-2xl">🧬</span>
                <div>
                  <div className="font-semibold text-gray-900">Longevity &amp; Performance Hub</div>
                  <div className="mt-1 text-sm text-gray-600">All the products and programs people buy to optimize healthspan</div>
                </div>
              </div>
            </Link>
            <Link href="/labs" className="rounded-lg border border-emerald-200 bg-white p-6 hover:shadow-md transition-shadow">
              <div className="flex items-start gap-3">
                <span className="text-2xl">🧪</span>
                <div>
                  <div className="font-semibold text-gray-900">At-Home Lab Testing</div>
                  <div className="mt-1 text-sm text-gray-600">Track the biomarkers behind metabolic health and longevity</div>
                </div>
              </div>
            </Link>
          </div>
